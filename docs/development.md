...
- **A page either calls the real API or it's a `PlaceholderPage`.** Never wire a page
  up to fabricated/hardcoded data to make it look done — Dashboard and Settings are
  `PlaceholderPage`s because their backend pieces (a combined account score; a weights
  read/write endpoint) don't exist yet, not because building the UI was skipped. See
